...
url <- "http://mfaapsny.org/en/allnews/news/visits/"
...
for(i in 1:page_nr) {
  ...
  printtext <- paste(i, "Abkhazia | Meetings", sep = ": ")
  print(printtext)
  
  scrape_url <- paste0("http://mfaapsny.org/en/allnews/news/visits/?PAGEN_1=", i)
  ...
  dfs[[i]] <- do.call(rbind, Map(data.frame,
                                 ...
                                 URL = scrape_url,
                                 News_type = "Meetings of the Minister",
...
readr::write_csv(DF, "01_ABKH_Meetings_of_Minister.csv")
