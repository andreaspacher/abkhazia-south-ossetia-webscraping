...
url <- "http://mfaapsny.org/en/allnews/news/statements_speeches/"
...
for(i in 1:page_nr) {
  ...
  printtext <- paste(i, "Abkhazia | Statements & Speeches", sep = ": ")
  print(printtext)
  
  scrape_url <- paste0("http://mfaapsny.org/en/allnews/news/statements_speeches/?PAGEN_1=", i)
  ...
  dfs[[i]] <- do.call(rbind, Map(data.frame,
                                 ...
                                 URL = scrape_url,
                                 News_type = "Statements and Speeches",
...
readr::write_csv(DF, "03_ABKH_Statements_and_Speeches.csv")
